...
def construct_grid_graph(listNodes, restricted_dic, margin, obstacle_weight=10000):
    ...
    graph.add_edges_from(
        [((x, y), (x + 1, y + 1)) for x in range((round(n_points / 2) + 1) - 1) for y in range(n_points - 1)] +
        [((x + 1, y), (x, y + 1)) for x in range((round(n_points / 2) + 1) - 1) for y in range(n_points - 1)],
        weight=listNodes["fur_neighbour"], r_cost=listNodes["fur_neighbour"])
...
def a_star_path_cost(listNodes, graph):
    try:
        path = nx.astar_path(graph, source=listNodes['start'], target=listNodes['end'], heuristic=energy_from_AtoB_MaxS,
                             weight='r_cost')
        # experiment
        total_dist = 0
