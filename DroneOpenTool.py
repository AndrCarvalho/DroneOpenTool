...
def main(workspace, name, start, goal, planner, output, gran, margin, restr, area, weight):
    ...
    if planner == "Online":
        online_runner(str(workspace +'\\'+ name), (float(start[1:-1].split(',')[0]), float(start[1:-1].split(',')[1])), (float(goal[1:-1].split(',')[0]), float(goal[1:-1].split(',')[1])), gran, margin, restr, weight)
    elif planner == "Offline":
        offline_runner(str(workspace +'\\'+ name), (float(start[1:-1].split(',')[0]), float(start[1:-1].split(',')[1])), (float(goal[1:-1].split(',')[0]), float(goal[1:-1].split(',')[1])), gran, margin, restr, area, weight)
    else:
...
if __name__ == "__main__":
    ...
    c_params.add_argument('--restr', metavar='restrictions', required=True, choices=['buildings',  'airways', 'residential', 'water', 'woods', 'military'], nargs="+", type=str,
    ...
    main(workspace=args.outputPath, name=args.mission, start=args.start, goal=args.goal, planner=args.planner, output=args.output, gran=args.gran, margin=args.margin, restr=args.restr, area=args.area, weight=args.obs_weight)
