...
def pre_compute():
    ...
    if distance_s_e >1000:
        zoom = 15
    if distance_s_e > 2600:
        zoom = 14
...
def compute():
    ...
    #some problems , not large or generic enough ###Solution same but for the bounds!!
    #area = (min(start[0],end[0])-0.01,min(start[1],end[1])-0.01,max(start[0],end[0])+0.01,max(start[1],end[1])+0.01)
    #area = rq.form.area

    for res in restric.split(','):
        if res == 'buildings':
            restrictions["buildings_nodes"], restrictions["buildings_ways"] = DC.collectBuildings_OSM_data(area)
            restricted_points["buildings"] = RC.inv_node_filter(missionGrid, restrictions["buildings_ways"])
        if res == 'water':
            restrictions["watArea_nodes"], restrictions["watArea_ways"] = DC.collectWaterAreas_OSM_data(area)
            restricted_points["watAreas"] = RC.inv_node_filter(missionGrid, restrictions["watArea_ways"])
        if res == 'residential':
            restrictions["resAreas_nodes"], restrictions["resAreas_ways"] = DC.collectResAreas_OSM_data(area)
            restricted_points["resAreas"] = RC.inv_node_filter(missionGrid, restrictions["resAreas_ways"])
        if res == 'woods':
            restrictions["woodsAreas_nodes"], restrictions["woodsAreas_ways"] = DC.collectWoodsAreas_OSM_data(area)
            restricted_points["woodsAreas"] = RC.inv_node_filter(missionGrid, restrictions["woodsAreas_ways"])
        if res == 'military':
            restrictions["militaryAreas_nodes"], restrictions["militaryAreas_ways"] = DC.collectMilitaryAreas_OSM_data(area)
            restricted_points["militaryAreas"] = RC.inv_node_filter(missionGrid, restrictions["militaryAreas_ways"])
        if res == 'aero':
            restrictions["airport_nodes"], restrictions["airport_ways"] = DC.collectAero_OSM_data(area)
